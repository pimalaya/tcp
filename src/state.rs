...
/// The stream I/O state, owned by flows and updated by handlers.
///
...
impl State {
    ...
    pub fn enqueue_bytes(&mut self, bytes: impl IntoIterator<Item = impl Deref<Target = u8>>) {
        let bytes = bytes.into_iter().map(|b| *b);
